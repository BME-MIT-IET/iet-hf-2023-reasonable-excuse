...
public class Tests {
	...
	static void PlayerStepsTest()
	{
		...
		else System.out.println("Nem fert�z�d�tt meg medvev�russal");
	...
	static void PickUpMaterialTest()
	{
		...
		{
			System.out.println("Siker�lt felvenni: "+((regiAminoHely-ujAminoHely)+(regiNucleoHely-ujNucleoHely))+" anyagot");
		}
	...
	static void LearnGeneticCodeTest()
	{
		...
		{
			System.out.println("Siker�lt megtanulni!");
			if(v.getGeneticCodes().size()==4) 
			{
				System.out.println(name+" megnyerte a j�t�kot!");
				Game.GetInstance().EndGame();
			}	
		}
		else System.out.println("Nem siker�lt megtanulni!");
	...
	static void PlayerDiesTest(){
		...
		Axe axe = new Axe();
		Field f = new FreeField();
		System.out.println("Virol�gus akin a fejsz�t haszn�lt�k: ");
		name = scan.nextLine();
		Virologist v = new Virologist(name);
		v.SetField(f);
		...
		if(Arrays.asList(f.GetVirologists()).contains(v)) System.out.println(v.getName() + " nem t�nt el.");
	...
	{
		BufferedReader lr = new BufferedReader(new InputStreamReader(System.in));
		System.out.print("Virol�gus neve: ");
		String name = "";
		try {
			name = lr.readLine();
		} catch (IOException e) {
		...
		case 1:
			...
			if(v.getAgentInventory().size()>regiGcDB) System.out.println("Siker�lt Amnesia agent k�sz�teni");
			else System.out.println("Nem siker�lt agent k�sz�teni");
			break;
		case 2:
			v.CreateAgent(v.getGeneticCodes().get(1));
			if(v.getAgentInventory().size()>regiGcDB) System.out.println("Siker�lt Dance agent k�sz�teni");
			else System.out.println("Nem siker�lt agent k�sz�teni");
			break;
		case 3:
			v.CreateAgent(v.getGeneticCodes().get(2));
			if(v.getAgentInventory().size()>regiGcDB) System.out.println("Siker�lt Paralyze agent k�sz�teni");
			else System.out.println("Nem siker�lt agent k�sz�teni");
			break;
		case 4:
			v.CreateAgent(v.getGeneticCodes().get(3));
			if(v.getAgentInventory().size()>regiGcDB) System.out.println("Siker�lt Protection agent k�sz�teni");
			else System.out.println("Nem siker�lt agent k�sz�teni");
			break;
	...
	static void UseAgentTest()
	{
		...
		case 1:
			...
			System.out.println("Sikeresen haszn�lva lett az Amnesia �gens a "+vn2+" nev� virol�guson");
			break;
		case 2:
			v.AddGeneticCode(new DanceGeneticCode());
			v.CreateAgent(v.getGeneticCodes().get(0));
			v.UseAgent(v2, v.getAgentInventory().get(0));
			System.out.println("Sikeresen haszn�lva lett az Dance �gens a "+vn2+" nev� virol�guson");
		...
		case 3:
			...
			v.CreateAgent(v.getGeneticCodes().get(0));
			v.UseAgent(v2, v.getAgentInventory().get(0));
			System.out.println("Sikeresen haszn�lva lett az Paralyze �gens a "+vn2+" nev� virol�guson");
		...
		case 4:
			...
			v.CreateAgent(v.getGeneticCodes().get(0));
			v.UseAgent(v2, v.getAgentInventory().get(0));
			System.out.println("Sikeresen haszn�lva lett az Protection �gens a "+vn2+" nev� virol�guson");
